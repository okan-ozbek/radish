...
private:
    ...
    void Process() {
        while (!m_input.empty() && !m_closeAfterWrite) {
            auto parsed = RespCodec::ParseCommand(m_input);
            if (parsed.status == RespParseResult::Status::Incomplete) {
                return;
            }
            if (parsed.status == RespParseResult::Status::Error) {
                Queue(RespCodec::Error(parsed.error), true);
                return;
            }

            m_input.erase(0, parsed.consumed);
            auto [response, close] = Dispatch(parsed.command);
    ...
    CommandResult HandleHello(const RespCommand& command, std::string_view name) {
        ...
        if (command.arguments[1] == "2") {
            ...
            return { RespCodec::Error("NOPROTO unsupported protocol version"), false };
        }
        return { RespCodec::Hello(m_version), false };
    }

    CommandResult HandlePing(const RespCommand& command, std::string_view name) {
        if (command.arguments.size() == 1) {
            return { RespCodec::SimpleString("PONG"), false };
        }
    ...
    CommandResult HandleGet(const RespCommand& command, std::string_view name) {
        if (command.arguments.size() != 2) {
            return WrongArity(name);
        }
        return { RespCodec::BulkString(m_database.Get(command.arguments[1]), m_version), false };
    }

    CommandResult HandleSet(const RespCommand& command, std::string_view name) {
        if (command.arguments.size() != 3) {
            return WrongArity(name);
        }
        m_database.Create(command.arguments[1], command.arguments[2]);
        return { RespCodec::SimpleString("OK"), false };
    }

    CommandResult HandleDelete(const RespCommand& command, std::string_view name) {
        if (command.arguments.size() < 2) {
            return WrongArity(name);
        }
        long long deleted{};
        ...
        return { RespCodec::Integer(deleted), false };
    }

    CommandResult HandleRename(const RespCommand& command, std::string_view name) {
        if (command.arguments.size() != 3) {
            return WrongArity(name);
        }
        if (!m_database.Rename(command.arguments[1], command.arguments[2])) {
            return { RespCodec::Error("no such key"), false };
        }
        return { RespCodec::SimpleString("OK"), false };
    }

    CommandResult HandleExists(const RespCommand& command, std::string_view name) {
        if (command.arguments.size() < 2) {
            return WrongArity(name);
        }
        long long exists{};
        ...
        return { RespCodec::Integer(exists), false };
    }

    CommandResult HandleKeys(const RespCommand& command, std::string_view name) {
        if (command.arguments.size() != 2) {
            return WrongArity(name);
        }
        if (command.arguments[1] != "*") {
            return { RespCodec::Error("only KEYS * is supported"), false };
        }
        return { RespCodec::Array(m_database.Scan(), m_version), false };
    }

    CommandResult HandleDatabaseSize(const RespCommand& command, std::string_view name) {
        if (command.arguments.size() != 1) {
            return WrongArity(name);
        }
        return { RespCodec::Integer(static_cast<long long>(m_database.Size())), false };
    }

    CommandResult HandleFlushDatabase(const RespCommand& command, std::string_view name) {
        if (command.arguments.size() != 1) {
            return WrongArity(name);
        }
        m_database.Clear();
    ...
    CommandResult HandleQuit(const RespCommand& command, std::string_view name) {
        ...
        return { RespCodec::SimpleString("OK"), true };
    }
    ...
    static CommandResult WrongArity(const std::string_view name) {
        return { RespCodec::Error("wrong number of arguments for '" + std::string(name) + "' command"), false };
    }
    ...
    void Write() {
        ...
        asio::async_write(m_socket, asio::buffer(m_output.front()), [self](const asio::error_code& error, const std::size_t) {
            ...
            if (!self->m_output.empty()) {
                ...
                const auto closeResult = self->m_socket.close(ignored);
                if (shutdownResult || closeResult) {
                    return;
                }
            }
        });
...
void TcpServer::Stop() {
    ...
    const auto closeResult = m_acceptor.close(ignored);
    if (cancelResult || closeResult) {
        return;
    }
}
